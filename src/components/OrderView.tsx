...
export function StatusTrail({ status }: { status: OrderStatus }) {
  ...
  return (
    <ol className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm font-bold">
      {STEPS.map((s, i) => (
        <li key={s} className={`flex items-center gap-2 ${i <= at ? "text-farm-800" : "text-ink/35"}`}>
...
export function ItemsTable({ items, total }: { items: OrderItem[]; total: number }) {
  return (
    <table className="w-full text-sm">
      <tbody className="divide-y divide-ink/10">
        {items.map((i, k) => (
          <tr key={k}>
            <td className="py-2 pr-2">{i.quantity} × {i.product_name ?? i.name}{i.unit && <span className="text-ink/50"> ({i.unit})</span>}</td>
