...
export default function Basket() {
  ...
  if (lines.length === 0) return (
    <div className="container-x py-16">
      ...
      <p className="mt-4 font-hand text-3xl text-ink/60">Nothing in here yet.</p>
    ...
    <div className="container-x py-10">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1.5fr_1fr]">
          ...
          {lines.map((l) => (
            <li key={l.product_id} className="flex flex-wrap items-center gap-4 py-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-ink/60">{formatRand(l.unit_price_cents)} {l.unit && `· ${l.unit}`}</p>
              </div>
              ...
              <button onClick={() => update(l.product_id, 0)} className="p-1 text-ink/50 hover:text-sun-500" aria-label={`Remove ${l.name}`}><X className="h-5 w-5" /></button>
            </li>
          ))}
        </ul>
        <aside className="self-start border-2 border-ink bg-white p-5">
          ...
          <p className="mt-2 text-xs text-ink/60">Final prices are confirmed when you place the order. Delivery, if you need it, is quoted separately.</p>
