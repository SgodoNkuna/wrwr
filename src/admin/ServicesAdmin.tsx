...
export default function ServicesAdmin() {
  ...
  return (
    <>
      ...
      <p className="mb-4 text-sm text-ink/70">Published services appear on the About page. None are offered yet, so add them here when you're ready.</p>
      <ErrorBox error={error} />
      {items.length === 0 && <p className="card p-6 text-sm text-ink/70">No services yet.</p>}
      <div className="space-y-3">
        {items.map((s, i) => (
          <div key={s.id || `new-${i}`} className="card grid gap-3 p-4 md:grid-cols-[1fr_2fr_auto_auto] md:items-end">
            <label className="block"><span className="label">Name</span><input className="input" maxLength={120} value={s.name} onChange={(e) => set(i, { name: e.target.value })} /></label>
            <label className="block"><span className="label">Description</span><input className="input" maxLength={3000} value={s.description ?? ""} onChange={(e) => set(i, { description: e.target.value })} /></label>
